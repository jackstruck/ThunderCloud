...
resource "google_cloud_run_v2_job" "backfill_inventory" {
  name                = "${var.name_prefix}-backfill-inventory"
  project             = var.project_id
  location            = var.region
  labels              = merge(var.labels, { purpose = "historical-backfill-audit" })
  ...
  template {
    ...
    template {
      ...
      containers {
        name    = "inventory"
        image   = var.backfill_inventory_image
        command = ["python", "-m", "worker.backfill_cloud_run"]
  ...
  depends_on = [
    ...
    google_secret_manager_secret_iam_member.batch_worker_csek_accessor,
    google_storage_bucket_iam_member.runtime_source_reader,
    google_storage_bucket_iam_member.runtime_backfill_audit_user,
  ]
}
